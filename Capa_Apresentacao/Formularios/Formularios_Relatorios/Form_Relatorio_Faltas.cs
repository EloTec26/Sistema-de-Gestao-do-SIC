...
{
    public partial class Form_Relatorio_Faltas : Form
    {
        private dados_relatorios_faltas _dados_relatorio_faltas;
        private List<Dominio_Relatorio_Faltas> _dominio_Relatorio_faltas;
        public Form_Relatorio_Faltas()
        {
            InitializeComponent();

            _dados_relatorio_faltas = new dados_relatorios_faltas();
            _dominio_Relatorio_faltas = new List<Dominio_Relatorio_Faltas>();

            reportViewer1.ProcessingMode = Microsoft.Reporting.WinForms.ProcessingMode.Local;
            reportViewer1.LocalReport.ReportEmbeddedResource =
                "Capa_Apresentacao.Formularios.Formularios_Relatorios.Report1_Faltas.rdlc";
            panel_personalizado.Visible = false;
            panel_butoes.Width = 0;
        }

        private void Form_Relatorio_Faltas_Load(object sender, EventArgs e)
        {
        ...
        {
            _dominio_Relatorio_faltas.Clear();
            DataTable dataTable = _dados_relatorio_faltas.relatorioFaltas(dataInicio, dataFim);
            foreach (DataRow Ler_Lista in dataTable.Rows)
            {
                _dominio_Relatorio_faltas.Add(new Dominio_Relatorio_Faltas
                {
                    Codigo = Convert.ToInt32(Ler_Lista["Codigo"].ToString()),
                    Funcionario = Ler_Lista["Funcionario"].ToString(),
                    Desc_Falta = Ler_Lista["Desc_Falta"].ToString(),
