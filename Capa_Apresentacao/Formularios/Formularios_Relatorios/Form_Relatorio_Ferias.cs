...
{
    public partial class Form_Relatorio_Ferias : Form
    {
        private dados_relatorios_ferias _dados_relatorio_ferias;
        private List<Dominio_Relatorio_Ferias> _dominio_Relatorio_Ferias;
        public Form_Relatorio_Ferias()
        {
            InitializeComponent();
            _dados_relatorio_ferias = new dados_relatorios_ferias();
            _dominio_Relatorio_Ferias = new List<Dominio_Relatorio_Ferias>();

            reportViewer1.ProcessingMode = Microsoft.Reporting.WinForms.ProcessingMode.Local;
            reportViewer1.LocalReport.ReportEmbeddedResource =
                "Capa_Apresentacao.Formularios.Formularios_Relatorios.Report_Ferias.rdlc";

            panel_personalizado.Visible = false;
            panel_butoes.Width = 0;
        }
        private void Form_Relatorio_Ferias_Load(object sender, EventArgs e)
        {
        ...
        private void CarregarDados(DateTime dataInicio, DateTime dataFim)
        {
            try
            {
                _dominio_Relatorio_Ferias.Clear();
                DataTable dataTable = _dados_relatorio_ferias.relatorioFerias(dataInicio, dataFim);
                foreach (DataRow Ler_Lista in dataTable.Rows)
                {
                    _dominio_Relatorio_Ferias.Add(new Dominio_Relatorio_Ferias
                    {
                        Código = Convert.ToInt32(Ler_Lista["Código"].ToString()),
                        Detalhes = Ler_Lista["Detalhes"].ToString(),
                        Detalhes_feria = Ler_Lista["Detalhes_feria"].ToString(),
